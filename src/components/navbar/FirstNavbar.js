    ...
    state={
        items: [
            {id:'000', name:'ABOUT US', href:'/about', items: [
                ...
                {id:'004', name:'MESSAGE', href:'/message'},
    ...
    render(){
       ...
       let navItems = this.state.items.map((navItem, index) =>{
            ...
            return (
                    ...
                    <li className={this.state.active === index? "Active" : null} key={navItem.id} onMouseLeave={() => this.setState({active: null})}>
                        <div>
                        {navItem.items === null?
                         <span><NavLink to={navItem.href} exact >{navItem.name}</NavLink></span>
                        : <NavLink to={navItem.href} exact onMouseEnter={()=> this.secondNavHandler(index)}
